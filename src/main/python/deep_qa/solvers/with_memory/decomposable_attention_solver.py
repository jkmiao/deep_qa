...
class DecomposableAttentionSolver(MemoryNetworkSolver):
    ...
    def _build_model(self):
        ...
        knowledge_embedding = KnowledgeSqueezer()(knowledge_embedding)
...
class KnowledgeSqueezer(Layer):
    '''
    Takes the embedding of (masked) knowledge whose length is 1 and squeezes it along the knowledge_length
    dimension.
    '''
    def __init__(self, **kwargs):
        self.supports_masking = True
        super(KnowledgeSqueezer, self).__init__(**kwargs)
    ...
    def compute_mask(self, x, mask=None):
        ...
        if mask is None:
            ...
            return mask[:, 0, :]  #(batch_size, sentense_length)
    ...
    def call(self, x, mask=None):
        return K.squeeze(x, 1)  # (batch_size, sentence_length, embed_dim)
